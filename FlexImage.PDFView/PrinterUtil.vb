...
Public Class PrinterUtil
    ...
    Public Shared Function CreateCustomPrintToFilePort() As String
        ...
        p1.pPortName = Path.GetTempPath & "\" & TempString & ".prn"
        CreateCustomPrintToFilePort = p1.pPortName
        PrinterInterOp.AddPortEx(Nothing, 1, p1, "Local Port")
    End Function

    Public Shared Function CreateNewTempPrinter(ByVal PrinterName As String, ByVal PortName As String, _
                                                ByVal PrintDriverName As String) As Integer
    ...
    Public Shared Sub PrintImageToPrinter(ByRef myFileName As String, ByVal myPrinterSettings As PrinterSettings, _
    ...
    Private Sub PrintDocument1_PrintPage(ByVal sender As Object, _
        ...
        Dim RenderDPI As Integer = 300
        'Set to 600 if this resolution is too low (speed vs. time)
        ...
        If (image.Height > image.Width And e.Graphics.VisibleClipBounds.Width > e.Graphics.VisibleClipBounds.Height) _
           Or (image.Width > image.Height And e.Graphics.VisibleClipBounds.Height > e.Graphics.VisibleClipBounds.Width) _
            Then
        ...
        If ScalePercentage < 0.75F Then _
            'Re-render the image to create a smaller print file and save printer processing time
            image = ImageUtil.GetImagePageFromFileForPrint(mFileName, mCurrentPage, OptimalDPI, mPassword)
            If (image.Height > image.Width And e.Graphics.VisibleClipBounds.Width > e.Graphics.VisibleClipBounds.Height) _
               Or _
               (image.Width > image.Height And e.Graphics.VisibleClipBounds.Height > e.Graphics.VisibleClipBounds.Width) _
                Then
                image.RotateFlip(RotateFlipType.Rotate270FlipNone)
            End If
        End If
...
Friend Class PrinterInterOp
    ...
    <StructLayout(LayoutKind.Sequential, CharSet := CharSet.Auto), SuppressUnmanagedCodeSecurity()> _
...
Friend Class RawPrinterHelper
    ...
    < _
        DllImport _
            ("winspool.Drv", EntryPoint := "OpenPrinterA", SetLastError := True, CharSet := CharSet.Ansi, _
             ExactSpelling := True, CallingConvention := CallingConvention.StdCall)> _
    Public Shared Function OpenPrinter(<MarshalAs(UnmanagedType.LPStr)> ByVal szPrinter As String, _
                                       ByRef hPrinter As IntPtr, ByVal pd As IntPtr) As Boolean
    End Function

    < _
        DllImport _
            ("winspool.Drv", EntryPoint := "ClosePrinter", SetLastError := True, ExactSpelling := True, _
    ...
    < _
        DllImport _
            ("winspool.Drv", EntryPoint := "StartDocPrinterA", SetLastError := True, CharSet := CharSet.Ansi, _
             ExactSpelling := True, CallingConvention := CallingConvention.StdCall)> _
    ...
    < _
        DllImport _
            ("winspool.Drv", EntryPoint := "EndDocPrinter", SetLastError := True, ExactSpelling := True, _
             CallingConvention := CallingConvention.StdCall)> _
    ...
    < _
        DllImport _
            ("winspool.Drv", EntryPoint := "StartPagePrinter", SetLastError := True, ExactSpelling := True, _
             CallingConvention := CallingConvention.StdCall)> _
    ...
    < _
        DllImport _
            ("winspool.Drv", EntryPoint := "EndPagePrinter", SetLastError := True, ExactSpelling := True, _
             CallingConvention := CallingConvention.StdCall)> _
    ...
    < _
        DllImport _
            ("winspool.Drv", EntryPoint := "WritePrinter", SetLastError := True, ExactSpelling := True, _
             CallingConvention := CallingConvention.StdCall)> _
    Public Shared Function WritePrinter(ByVal hPrinter As IntPtr, ByVal pBytes As IntPtr, ByVal dwCount As Int32, _
    ...
    Public Function SendBytesToPrinter(ByVal szPrinterName As String, ByVal pBytes As IntPtr, ByVal dwCount As Int32) _
        As Boolean
    ...
    Public Function SendFileToPrinter(ByVal szPrinterName As String, ByVal szFileName As String) As Boolean
        ...
        nLength = Convert.ToInt64(fs.Length)
