...
OCRCurrentImage:
  ...
  End Sub
    ...
    Private Sub tsPrevious_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        DoPreviousPage()
    End Sub

    Private Sub tsNext_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        DoNextPage()
    End Sub
    ...
    Private Sub tsZoomIn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        DoZoomIn()
    End Sub

    'Private Sub tsPageNum_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs)
    ...
    'Private Sub tsPageNum_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '    mCurrentPageNumber = tsPageNum.Text
    '    DisplayCurrentPage()
    'End Sub
...
#Region "Functions"
    ...
    Public Function GetPageCount() As Integer
        GetPageCount = mPDFPageCount
    End Function

    Public Function GetCurrentPage() As Integer
        GetCurrentPage = mCurrentPageNumber
    End Function

    Public Sub DoSave(ByVal newFileName As String)
        
    End Sub

    Public Sub DoGotoPage(ByVal pageNum As Integer)
        mCurrentPageNumber = pageNum
        DisplayCurrentPage()
    End Sub
    ...
    Public Function ExportToTif(ByVal filename As String, ByVal pageNumber As Integer) As String
        ...
        outFileName = filename.Replace(extension, "_" & pageNumber.ToString() + ".TIF")
        returnFileName = ConvertPDF.PDFConvert.ConvertPdfToGraphic(filename, outFileName, ExportOptions.BW_TIFF_G4, 200, pageNumber, pageNumber, False, "")

        ExportToTif = returnFileName
    End Function

    Public Function ExplodeTif(ByVal filename As String, ByVal targetDir As String) As String()

        Dim returnFileName As String
        Dim outFileName As String
        Dim files(mPDFPageCount - 1) As String
        Dim extension As String = System.IO.Path.GetExtension(filename)

        Try
            System.IO.Directory.Delete(System.IO.Path.GetDirectoryName(filename), True)
        Catch ex As Exception
            'do nothing
        End Try

        If (targetDir = "") Then
            targetDir = System.IO.Directory.GetCurrentDirectory() + "\\OCR\\"
        End If

        For i As Integer = 1 To mPDFPageCount
            outFileName = targetDir + System.IO.Path.GetFileNameWithoutExtension(filename) + "\\" + System.IO.Path.GetFileNameWithoutExtension(filename) + "_" + i.ToString() + ".TIF"


            System.IO.Directory.CreateDirectory(System.IO.Path.GetDirectoryName(outFileName))
            returnFileName = ConvertPDF.PDFConvert.ConvertPdfToGraphic(filename, outFileName, ExportOptions.BW_TIFF_G4, 300, i, i, False, "")
            files(i - 1) = outFileName
        Next
    ...
    Public Function ExplodeJPGSeq(ByVal filename As String, ByVal destPath As String, ByVal page As Integer) As String()
        ...
        For i As Integer = 1 To mPDFPageCount
            outFileName = destPath + String.Format("{0:00000}", i) + ".JPG"

            System.IO.Directory.CreateDirectory(System.IO.Path.GetDirectoryName(outFileName))

            If ((page = 0) Or (page = i)) Then
                returnFileName = ConvertPDF.PDFConvert.ConvertPdfToGraphic(filename, outFileName, ExportOptions.COLOR_JPEG, 200, i, i, False, "")
                files(i - 1) = outFileName
            End If
        Next
...
#End Region
    ...
    Private Sub tsPageNum_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
